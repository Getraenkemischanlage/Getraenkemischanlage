...
class HX711:
    ...
    def convertFromTwosComplement24bit(self, inputValue):
        if inputValue & 0x800000:
            inputValue = -((~inputValue & 0xFFFFFF) + 1)
        return inputValue
    ...
    def set_gain(self, gain):
        if gain == 128:
            ...
            raise ValueError("HX711::set_gain(): gain must be 128, 64, or 32!")
    ...
    def read_average(self, times=3):
        ...
        if len(valueList) == 0:
            raise ValueError("No values left after trimming outliers in read_average()")
    ...
    def get_reference_unit(self):
        return self.get_reference_unit_A()  # Korrektur: self. hinzugefügt
...
def hx711_add_event_detect(hx711_instance, event_callback):
    GPIO.add_event_detect(hx711_instance.DOUT, GPIO.FALLING, 
        callback=event_callback)  # Korrektur: hx711_instance statt self
